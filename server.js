...
const DATASETS_VALIDOS = ['bajas-dataset', 'mtp-dataset', 'altas-dataset', 'mujeres-dataset', 'semaforo-dataset'];
...
// Los datasets no comparten una misma forma: bajas/mtp/altas guardan { records: [...] },
// % Mujeres guarda { meses: {...} } y Sucesión guarda { posiciones: [...], personas: [...] }.
// Acá solo validamos que venga un objeto con contenido real en al menos una de esas formas.
function datasetTieneContenido(parsed) {
  if (!parsed || typeof parsed !== 'object') return false;
  if (Array.isArray(parsed.records) && parsed.records.length) return true;
  if (Array.isArray(parsed.posiciones) && parsed.posiciones.length) return true;
  if (parsed.meses && typeof parsed.meses === 'object' && Object.keys(parsed.meses).length) return true;
  return false;
}
app.put('/api/datos/:clave', requiereAdmin, claveValida, async (req, res) => {
  ...
  if (!datasetTieneContenido(parsed)) {
...
const DATASET_LABELS = {
  ...
  'mujeres-dataset': '% Mujeres (dotación por planta)',
  'semaforo-dataset': 'Sucesión de posiciones (semáforo)',
};
